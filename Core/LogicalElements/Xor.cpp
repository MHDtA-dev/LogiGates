...
namespace LogiGates::Core::LogicalElements {
    ...
    Xor::Xor(UI::Workspace *workspace) : Base(workspace) {
        this->typeName = "xor";
        ...
        pins.push_back(new Pin(this, PinType::OUTPUT,
                               "A " + UI::Localization::localization[UI::Localization::currentLocalization]["xor"] +
                               " " + UI::Localization::localization[UI::Localization::currentLocalization]["b"]));
    }

    void Xor::render() {
        ImNodes::BeginNode(id);
        ImGui::Image((ImTextureID) UI::Images::icons["xor"]->texture, {70, 70});
